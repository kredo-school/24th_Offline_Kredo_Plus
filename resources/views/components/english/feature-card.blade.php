@props(['icon' => 'star', 'title' => '', 'description' => '', 'href' => '#', 'progress' => null, 'badge' => null, 'actionLabel' => null, 'color' => 'orange'])

@php
    // 英語学習モジュール全体で統一したオレンジ基調のアクセント
    // （文字色はorange-600でコントラストを確保し、視認性に配慮）
    $accent = [
        'icon' => 'bg-gradient-to-br from-orange-100 to-amber-50 ring-1 ring-orange-200 text-orange-600',
        'text' => 'text-orange-600',
        'bar' => 'bg-[#b95827]',
        'track' => 'bg-orange-100/70',
        'badge' => 'bg-[#b95827] text-white',
    ];
@endphp
...
<a href="{{ $href }}" class="bg-surface-container-lowest rounded-[0.75rem] shadow-sm hover:shadow-md transition-all p-6 flex flex-col gap-3 group no-underline text-inherit">
    ...
    <div class="mt-auto pt-1">
        ...
        <div class="w-full {{ $accent['track'] }} rounded-[0.75rem] h-1.5 overflow-hidden">
            <div class="{{ $accent['bar'] }} h-full rounded-[0.75rem]" style="width: {{ $progress }}%"></div>
        </div>
    </div>
    @endif

    <div class="flex items-center gap-1 {{ $accent['text'] }} text-label-md font-semibold {{ $progress === null ? 'mt-auto' : '' }} group-hover:gap-2 transition-all">
