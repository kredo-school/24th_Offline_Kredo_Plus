    ...
    <aside id="sidebar" class="hidden md:block w-60 shrink-0">
      <div class="sticky top-24">
        ...
        <nav class="flex flex-col gap-1 pl-5">
          <a href="{{ route('information.dynamic', $section->key) }}" style="--cat-color:{{ $section->color() }}" class="cat-link flex items-center justify-between py-2 text-sm font-medium text-[#241E1A]/40 hover:text-[#241E1A] @if(!$currentArea) active @endif">All</a>
          ...
          @foreach ($subCategories as $cat)
            <a href="{{ route('information.dynamic.show', [$section->key, $cat->slug]) }}"
               ...
               class="cat-link flex items-center justify-between py-2 text-sm font-medium text-[#241E1A]/40 hover:text-[#241E1A] @if($currentArea && $currentArea->id === $cat->id) active @endif">
              {{ $cat->name }}
            </a>
          @endforeach
        </nav>
