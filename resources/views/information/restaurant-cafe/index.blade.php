...
<div class="min-h-screen flex flex-col">
  ...
  <div class="max-w-container-max mx-auto w-full px-margin-mobile md:px-margin-desktop pt-8 md:pt-12">
    ...
    @php
      ...
      $defaultHeroDescription = $section->description ?? 'ゆったり座れるディナーも、居心地の良いカフェも、ここに全部揃ってます。';
    ...
    <section id="heroImg" class="relative overflow-hidden rounded-[0.75rem] mb-0 min-h-[280px] sm:min-h-[340px] md:min-h-[380px] p-8 md:p-10 bg-cover bg-center shadow-[0_1px_2px_rgba(36,30,26,0.06),0_8px_24px_-12px_rgba(36,30,26,0.18)]"
      ...
      <div class="relative flex flex-col lg:flex-row gap-8 w-full">
        <div class="flex-1">
          ...
          <p id="heroTitle" class="text-headline-md font-bold mb-3 drop-shadow-[0_1px_3px_rgba(255,255,255,0.9)]" style="color: {{ $section?->textColor() ?? '#e05237' }}">{{ $initialHeroTitle }}</p>
  ...
  <div id="mainCatNav" class="no-scrollbar grid grid-flow-col grid-rows-2 sm:grid-rows-1 auto-cols-[calc(50%-0.375rem)] sm:auto-cols-[calc(25%-0.5625rem)] gap-3 overflow-x-auto pb-1" style="scrollbar-width:none;">
    @foreach ($allMainCategories as $mc)
      @php
        ...
        $isCurrentMain = $mc->key === 'restaurant-cafe';
      @endphp
    ...
    <aside id="sidebar" class="hidden md:block w-60 shrink-0">
      <div class="sticky top-24">
        ...
        <nav class="flex flex-col gap-1 pl-5" id="storeNav">
          <a href="#" style="--cat-color:{{ $section?->color() ?? '#e05237' }}" class="cat-link flex items-center justify-between py-2 text-sm font-medium text-[#241E1A]/40 hover:text-[#241E1A] {{ $initialCategory === null ? 'active' : '' }}">All</a>
          {{-- サブカテゴリー一覧はDBから動的に取得。アドミンが追加しても、この@foreachがそのまま対応する --}}
          {{-- 左のガイドはAllがメインカテゴリーの色、下に行くほど薄くなるグラデーション --}}
          @foreach ($subCategories as $cat)
            <a href="#" data-tag="{{ $cat->name }}" style="--cat-color:{{ \App\Models\Category::lighten($section?->color() ?? '#e05237', \App\Models\Category::sidebarTint($loop->index)) }}" class="cat-link flex items-center justify-between py-2 text-sm font-medium text-[#241E1A]/40 hover:text-[#241E1A] {{ $initialCategory === $cat->name ? 'active' : '' }}">{{ $cat->name }}</a>
          @endforeach
        </nav>
      </div>
    </aside>
