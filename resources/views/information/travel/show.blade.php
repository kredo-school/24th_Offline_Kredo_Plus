...
<title>{{ $currentArea?->name ?? 'All Areas' }} — Kredo Plus Travel</title>
  ...
  <div class="max-w-7xl w-full mx-auto px-4 sm:px-6 pt-3 flex justify-end">
    <label class="flex items-center gap-2 text-xs text-ink/50 bg-ink/[0.03] px-3 py-1.5 rounded-full">
      ...
      <select id="currentUserSelect" class="bg-transparent font-semibold text-ink outline-none cursor-pointer">
        ...
        <option value="Mika S.">Mika S.</option>
        <option value="Grace P.">Grace P.</option>
  ...
  <div class="max-w-7xl w-full mx-auto px-4 sm:px-6 pt-6">
    ...
    <div class="relative h-52 sm:h-64 rounded-3xl overflow-hidden shadow-card">
      ...
      <div class="relative h-full flex flex-col justify-end p-6 sm:p-8">
        <h1 class="font-display text-4xl sm:text-5xl font-bold text-white">{{ $currentArea->name ?? 'Travel & Tourism' }}</h1>
        <p class="text-white/85 mt-1 text-sm sm:text-base">{{ $currentArea->description ?? 'セブの海・自然・カルチャーを、まるごと満喫しよう。' }}</p>
      </div>
    </div>
  </div>
  ...
  <div class="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 py-8 flex gap-6">
    ...
    <aside id="sidebar" class="hidden md:block w-60 shrink-0">
      <div class="sticky top-24">
        ...
        <p class="font-mono text-[11px] tracking-[0.18em] text-ink/40 mb-3 pl-1">AREA</p>
        <nav class="flex flex-col gap-1 pl-5" id="areaNav">
          ...
          @foreach ($areas as $area)
            <a href="{{ route('travel.show', $area->slug) }}"
               ...
               class="cat-link flex items-center justify-between py-2 text-sm text-ink/70 hover:text-ink @if($currentArea && $currentArea->id === $area->id) active @endif">
              {{ $area->name }}
            </a>
          @endforeach
          <a href="{{ route('travel.index') }}" style="--cat-color:#7c6ba6" class="cat-link flex items-center justify-between py-2 text-sm text-ink/70 hover:text-ink @if(!$currentArea) active @endif">All Areas</a>
        </nav>
      </div>
    </aside>
...
<nav class="fixed bottom-0 w-full z-50 bg-white shadow-[0_-4px_20px_-4px_rgba(30,58,138,0.15)] flex justify-around items-center h-20 px-4 pb-2 border-t border-slate-100">
  <a href="{{ route('travel.index') }}" class="flex flex-col items-center justify-center gap-1 text-[#2f5fdb] px-4 py-1 active:scale-90 transition-all duration-200">
