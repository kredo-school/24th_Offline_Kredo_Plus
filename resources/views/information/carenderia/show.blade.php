  ...
  <header class="sticky top-0 z-30 bg-paper/90 backdrop-blur border-b border-ink/10">
    <div class="max-w-3xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between">
      <div class="flex items-center gap-3">
        <a href="{{ route('carenderia.index') }}" class="p-2 -ml-2 rounded-lg hover:bg-ink/5 transition-colors" aria-label="戻る">
      ...
      @if ($post->user_id === auth()->id() || (bool) auth()->user()?->is_admin)
        <div class="flex items-center gap-1">
          @if ($post->user_id === auth()->id())
            <a href="{{ route('carenderia.edit', $post) }}"
          ...
          @endif
          <form id="deleteForm" action="{{ route('carenderia.destroy', $post) }}" method="POST">
            @csrf
            @method('DELETE')
