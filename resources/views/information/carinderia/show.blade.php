      ...
      @if ($post->user_id === auth()->id() || (bool) auth()->user()?->is_admin)
        <div class="flex items-center gap-1">
          @if ($post->user_id === auth()->id())
            <a href="{{ route('carinderia.edit', $post) }}"
          ...
          @endif
          <form id="deleteForm" action="{{ route('carinderia.destroy', $post) }}" method="POST">
            @csrf
            @method('DELETE')
...
<nav class="fixed bottom-0 w-full z-50 bg-white shadow-[0_-4px_20px_-4px_rgba(30,58,138,0.15)] flex justify-around items-center h-20 px-4 pb-2 border-t border-slate-100">
  <a href="{{ route('carinderia.index') }}" class="flex flex-col items-center justify-center gap-1 text-[#2f5fdb] px-4 py-1 active:scale-90 transition-all duration-200">
