...
<div class="flex-grow max-w-container-max mx-auto w-full px-margin-mobile md:px-margin-desktop py-12">
    ...
    <x-english.breadcrumb>
        ...
        <span class="mx-1">/</span>
        <span class="text-blue-950/90 font-semibold">結果</span>
    </x-english.breadcrumb>
    ...
    <div class="max-w-2xl mx-auto space-y-6">
        ...
        <div class="bg-surface-container-lowest rounded-[0.75rem] shadow-sm p-8 text-center">
            ...
            <h1 class="text-headline-lg font-bold text-blue-950/90 mb-2">{{ $material->category->name }}</h1>
            ...
            @if($bestWpm > 0)
            <div class="mb-4 text-caption text-blue-950/90">
                ベスト記録: <span class="font-bold text-blue-950/90">{{ $bestWpm }} WPM</span>
                @if($record->wpm >= $bestWpm)
                <span class="ml-2 text-orange-600 font-bold inline-flex items-center gap-1 align-middle">
                    <span class="material-symbols-outlined !text-base">emoji_events</span>新記録！
                </span>
                @endif
            </div>
            @endif
