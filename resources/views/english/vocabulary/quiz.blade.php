...
@section('title', '語彙クイズ - ' . strtoupper(explode('-', $level)[0]) . ' ' . explode('-', $level)[1])
...
<div class="flex-grow max-w-container-max mx-auto w-full px-margin-mobile md:px-margin-desktop py-12">
    ...
    <div x-data="vocabQuizApp({
            questions: {{ json_encode($questions) }},
            submitUrl: '{{ route('english.vocabulary.quiz.submit', $level) }}'
        })">

        {{-- ヘッダー --}}
        <div class="mb-6">
            <div class="flex items-center justify-between mb-2">
                <h1 class="text-headline-md font-bold text-blue-950/90">{{ $levelLabel }} 語彙クイズ</h1>
