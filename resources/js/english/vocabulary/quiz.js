...
/**
 * Alpine.data component for Vocabulary Quiz (S15).
 * Answers are judged locally for immediate feedback.
 * All answers are POSTed at the end (XP awarded server-side).
 *
 * Config:
 *   questions – [{id, word, correct, options: string[]}]
 *   submitUrl – POST /english/vocabulary/{level}/quiz/submit
 */
export function vocabQuizApp(config) {
    return {
        questions:    config.questions ?? [],
        submitUrl:    config.submitUrl,

        currentIndex:   0,
        selectedOption: null,   // index into current.options
        isAnswered:     false,
        isCorrect:      false,
        score:          0,
        isComplete:     false,
